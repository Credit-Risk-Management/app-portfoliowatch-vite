...
import { NEW_FINANCIAL_UPLOAD_TABLE_HEADERS, NEW_FINANCIAL_SECTION_ROWS } from './_helpers/newFinancialUploadTable.consts';
import * as tableEvents from './_helpers/newFinancialUploadTable.events';
import './NewFinancialUploadTable.scss';

export default function NewFinancialUploadTable() {
  const { documentsByType, isLoading: modalLoading } = $modalState.value;

  const uploadTableRows = useMemo(() => NEW_FINANCIAL_SECTION_ROWS.map(({ sectionId, title }) => {
      ...
      action: (
        <div role="presentation" onClick={(e) => e.stopPropagation()}>
          ...
          {hasFile ? (
            ...
            <label
              htmlFor={inputId}
              className="new-financial-upload-table__upload-label text-info-100 small fw-bold mb-0"
            >
              Upload
            </label>
          )}
