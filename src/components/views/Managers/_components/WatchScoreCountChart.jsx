  ...
  return (
    <UniversalCard headerText="Loans by WATCH Score (Count)">
      <ResponsiveContainer width="100%" height={240}>
        <PieChart>
          <Pie
            data={metrics.watchScoreCountData || []}
            cx="50%"
            cy="50%"
            labelLine={false}
            fill="#8884d8"
            ...
            label={false}
          >
            {(metrics.watchScoreCountData || []).map((entry, index) => (
              <Cell key={`cell-${index}`} fill={consts.getWatchScoreColor(entry.rating)} />
            ))}
          </Pie>
        </PieChart>
      </ResponsiveContainer>
      <WatchScoreLegend data={metrics.watchScoreCountData || []} metric="count" />
    </UniversalCard>
  );
};

export default WatchScoreCountChart;
