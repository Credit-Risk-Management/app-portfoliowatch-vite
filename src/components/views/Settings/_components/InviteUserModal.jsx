import { Form, Row, Col } from 'react-bootstrap';
import UniversalModal from '@src/components/global/UniversalModal';
import UniversalInput from '@src/components/global/Inputs/UniversalInput';
import SelectInput from '@src/components/global/Inputs/SelectInput';
import { $settingsView, $usersForm } from '@src/signals';
import { handleInviteUser } from '@src/components/views/Users/_helpers/users.events';
import Loadable from '@src/components/global/Loadable';

const ROLE_OPTIONS = [
  { value: 'USER', label: 'Basic User' },
  { value: 'ADMIN', label: 'Admin' },
];

const InviteUserModal = () => (
  <UniversalModal
    show={$settingsView.value.showInviteModal}
    onHide={() => {
      $settingsView.update({ showInviteModal: false });
      $usersForm.reset();
    }}
    headerText="Invite User"
    leftBtnText="Cancel"
    rightBtnText={$settingsView.value?.isLoading ? 'Sending...' : 'Send Invitation'}
    rightBtnOnClick={handleInviteUser}
    rightButtonDisabled={$settingsView.value?.isLoading}
  >
    <Form>
      <Loadable signal={$settingsView} template="component">
        <Row>
          <Col md={12} className="mb-16">
            <UniversalInput
              label="Email"
              type="email"
              value={$usersForm.value.email || ''}
              onChange={(e) => $usersForm.update({ email: e.target.value })}
              placeholder="user@example.com"
              required
              name="email"
            />
          </Col>
        </Row>
        <Row>
          <Col md={12} className="mb-16">
            <Form.Label>Role</Form.Label>
            <SelectInput
              name="role"
              signal={$usersForm}
              options={ROLE_OPTIONS}
              value={($usersForm.value.role || 'USER')}
              onChange={(option) => $usersForm.update({ role: option?.value || 'USER' })}
              notClearable
            />
          </Col>
        </Row>
        <Row>
          <Col md={12} className="mb-16">
            <Form.Label>Message (Optional)</Form.Label>
            <Form.Control
              as="textarea"
              rows={3}
              value={$usersForm.value.message || ''}
              onChange={(e) => $usersForm.update({ message: e.target.value })}
              placeholder="Add a personal message to the invitation..."
              name="message"
            />
          </Col>
        </Row>
      </Loadable>
    </Form>
  </UniversalModal>
);

export default InviteUserModal;
